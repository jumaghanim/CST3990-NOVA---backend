...
console.log("ENV CHECK → MONGO_URI exists:", !!MONGO_URI);
...
function buildReceipt(order) {
    ...
    return {
        ...
        items: order.items,
        subtotal: Number(order.subtotal || 0).toLocaleString(undefined, { maximumFractionDigits: 2 }),
        promoCode: order.promoCode || "",
        promoDiscountAmount: Number(order.promoDiscountAmount || 0).toLocaleString(undefined, { maximumFractionDigits: 2 }),
        totalPrice: Number(order.totalPrice).toLocaleString(undefined, { maximumFractionDigits: 2 })
    };
}
...
app.post("/api/orders", async (req, res) => {
    try {
        ...
        for (const item of cart) {
            ...
            subtotal += computedPrice;
        }

        const safePromoDiscountAmount = Number(promoDiscountAmount || 0);
        const safeFinalTotalPrice = Number(finalTotalPrice || subtotal);
        ...
        const order = new Order({
            ...
            promoDiscountAmount: safePromoDiscountAmount,
            totalPrice: safeFinalTotalPrice,
